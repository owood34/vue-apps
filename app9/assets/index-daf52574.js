...
const _sfc_main$1 = {
  ...
  setup(__props) {
    ...
    watch(
      email,
      () => error_email.value.innerHTML = email.value.match(/^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$/) ? "&nbsp;" : "Invalid Email"
    );
    watch(
      phone,
      () => error_phone.value.innerHTML = phone.value.match(/^\(?([0-9]{3})\)?[-. ]?([0-9]{3})[-. ]?([0-9]{4})$/) ? "&nbsp;" : "Invalid Phone Number Format: (###-###-####)"
    );
    ...
    function disable() {
      if (error_user.value != null && error_password.value != null && error_password2.value != null && error_email.value != null && error_phone.value != null) {
        return !(error_user.value.innerHTML === "&nbsp;" && error_user.value.innerHTML === error_password.value.innerHTML && error_password.value.innerHTML === error_password2.value.innerHTML && error_password2.value.innerHTML === error_email.value.innerHTML && error_email.value.innerHTML === error_phone.value.innerHTML);
      }
      return true;
    }
    function submit() {
...
const _sfc_main = {
  components: {
    CreateAccount: _sfc_main$1
  }
};
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  const _component_CreateAccount = resolveComponent("CreateAccount");
  return openBlock(), createBlock(_component_CreateAccount);
}
